...
if (!dbUrl) {
  ...
  console.error('Définis-la avec SUPABASE_PROJECT_ID et SUPABASE_PROJECT_REF_CONFIRMATION pour cibler explicitement la base voulue.');
  process.exit(1);
}
...
try {
  ...
  console.error(
    `\x1b[31m[supabase-db-push]\x1b[0m ${error instanceof Error ? error.message : error}`,
  );
  process.exit(1);
}
