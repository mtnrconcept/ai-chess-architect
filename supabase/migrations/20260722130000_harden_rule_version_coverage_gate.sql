...
begin
  ...
  if exists (
    ...
    where btrim(coalesce(evidence_path #>> '{}', ''))
  ...
  if exists (
    ...
    where pg_catalog.jsonb_path_exists(
        new.blueprint_json,
        ('strict ' || btrim(evidence_path #>> '{}'))::pg_catalog.jsonpath,
        '{}'::jsonb,
        true
...
-- Historical rows fall into two deliberately separate populations:
--   * contract v1 rows claimed complete coverage and must pass the exact
--     insert guard below;
--   * rows created before the coverage contract existed remain explicitly
--     uncertified, but must still prove that they came from their immutable
--     compilation source.
-- Any explicit contract value other than numeric v1 is neither legacy nor a
-- supported claim, so fail closed instead of silently downgrading it.
do $historical_coverage_audit$
declare
  v_contract_v1_count bigint;
  v_legacy_precontract_count bigint;
begin
  if exists (
    select 1
    ...
    where version.validation #> '{metrics,coverageContractVersion}'
        is not null
      and version.validation #> '{metrics,coverageContractVersion}'
        is distinct from '1'::jsonb
  ) then
    raise exception 'RULE_VERSION_HISTORICAL_COVERAGE_CONTRACT_UNSUPPORTED'
      using errcode = '23514';
  end if;
  ...
  into v_contract_v1_count, v_legacy_precontract_count
  from public.rule_versions as version;

  raise notice
    'RULE_VERSION_HISTORICAL_COVERAGE_AUDIT contract_v1=%, legacy_precontract_uncertified=%',
    v_contract_v1_count,
    v_legacy_precontract_count;
end;
$historical_coverage_audit$;

-- Re-run the exact insert guard only against rows that explicitly claim the
-- v1 contract. The temporary table has the same row shape but no copied
-- trigger, so a false certification claim aborts the migration.
create temporary table rule_versions_coverage_historical_audit
  (like public.rule_versions including defaults)
  on commit drop;

create trigger rule_versions_coverage_historical_audit_gate
before insert on rule_versions_coverage_historical_audit
for each row execute function public.enforce_rule_version_coverage_gate();

insert into rule_versions_coverage_historical_audit
select *
from public.rule_versions as version
where version.validation #> '{metrics,coverageContractVersion}'
  is not distinct from '1'::jsonb;

drop table rule_versions_coverage_historical_audit;
