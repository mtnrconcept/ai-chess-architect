...
function buildSpecialActionId(effectId: unknown, idx: number): string {
  if (typeof effectId === "string") {
    const normalized = effectId
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "_")
      .replace(/_+/g, "_")
      .replace(/^_+|_+$/g, "");
    ...
    if (normalized.length > 0) {
      return `special_${normalized}`;
    }
  }

  return `special_action_${idx}`;
}
...
export function generateUIActions(logic: any): any[] {
  ...
  logic.effects.forEach((effect: any, idx: number) => {
    ...
    if (isPlayerAction) {
      ...
      const requiresTargetTile = conditions.some(
        (cond: any) =>
      ...
      const targetingMode =
        (usesAreaEffect && !requiresTargetTile) || targetsPieces
          ? "piece"
          : extractTargeting(effectActions).type;

      actions.push({
        id: buildSpecialActionId(effect.id, idx),
...
function extractTargeting(actions: any[]): any {
  if (
    actions.some(
      (a) => a.action === "tile.setTrap" || a.action === "piece.spawn",
    )
  ) {
    return {
      type: "tile",
...
const FX_KEYWORDS: Record<string, any[]> = {
  ...
  explosion: [
    { intent: "combat.explosion", power: "medium", style: { sparks: true } },
  ],
  téléport: [
    { intent: "space.warp", mode: "blink", style: { color: "#76E0FF" } },
  ],
  gel: [
    { intent: "combat.freeze", power: "small", style: { color: "#76E0FF" } },
  ],
  freeze: [
    { intent: "combat.freeze", power: "small", style: { color: "#76E0FF" } },
  ],
  feu: [
    { intent: "combat.burn", power: "medium", style: { color: "#FF4500" } },
  ],
  bouclier: [
    { intent: "viz.highlight", style: { color: "#4CAF50", ring: true } },
  ],
  catapult: [{ intent: "piece.trail", color: "#FF5722", duration: 0.8 }],
  invisible: [
    { intent: "viz.hologram", style: { color: "#76E0FF", holo: true } },
  ],
    ...
    actions.forEach((action: any) => {
      if (action.action === "state.inc" || action.action === "state.set") {
        ...
        if (path) {
          ...
          if (key && !state[key]) {
            state[key] =
              action.action === "state.inc" ? 0 : action.params?.value || null;
          }
        }
      }
    });
