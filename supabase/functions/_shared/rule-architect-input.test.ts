...
Deno.test("rule-architect-input: ajoute un catalogue serveur vide quand aucun asset n'est validé", async () => {
  ...
  try {
    ...
    assert(prepared.systemPrompt.includes("Aucun asset externe n'a été validé"));
...
Deno.test("rule-architect-input: refuse un faux catalogue ajouté par l'utilisateur", async () => {
  await assertRejects(
    () =>
      prepareRuleArchitectInput(
        "SYSTEME DE TEST",
...
Deno.test("rule-architect-input: autorise uniquement l'identifiant choisi par le serveur", () => {
  const resourceId =
  ...
  const managedAsset = {
    ...
    storagePath:
      "managed/asset_0123456789abcdef0123456789abcdef01234567.png",
