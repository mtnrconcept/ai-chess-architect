...
const sanitizeConditions = (
  ...
  return source
    .map((condition) => {
      ...
      const hasType =
        typeof typedCondition.type === "string" &&
        typedCondition.type.length > 0;
      const operator =
        typedCondition.operator && allowedOperators.has(typedCondition.operator)
...
const sanitizeEffects = (value: unknown, issues: string[]): RuleEffect[] => {
  if (!value) return [];
  ...
  return source
    .map((effect) => {
      ...
      const hasAction =
        typeof typedEffect.action === "string" && typedEffect.action.length > 0;
      const hasTarget =
        typeof typedEffect.target === "string" && typedEffect.target.length > 0;
      ...
      return {
        ...
        target: isEffectTarget(typedEffect.target)
          ? typedEffect.target
          : "self",
...
export const analyzeRuleLogic = (rule: unknown): RuleAnalysisResult => {
  ...
  if (!isRecord(rule)) {
    issues.push(
      "Structure de règle invalide remplacée par des valeurs par défaut.",
    );
  }
  ...
  const rawRuleId =
    getRecordValue<unknown>(rawRule, "ruleId") ??
  ...
  const ruleId =
    typeof rawRuleId === "string" && rawRuleId.trim().length > 0
      ...
      : (() => {
          issues.push(
            "Identifiant de règle manquant ou invalide généré automatiquement.",
          );
          return fallbackRule.ruleId;
        })();

  const rawRuleName =
    getRecordValue<unknown>(rawRule, "ruleName") ??
    getRecordValue<unknown>(rawRule, "rule_name");
  const ruleName =
    typeof rawRuleName === "string" && rawRuleName.trim().length > 0
      ? rawRuleName.trim()
      : "Règle sans nom";
  ...
  const category =
    ...
    allowedCategories.includes(rawCategory as ChessRule["category"])
  ...
  if (
    ...
    if (
      ...
      console.warn(
        `[ruleValidation] Catégorie "${rawCategory}" normalisée vers "${category}"`,
      );
    }
  }
  ...
  const trigger =
    ...
    allowedTriggers.includes(rawTrigger as ChessRule["trigger"])
  ...
  const rawAffectedPieces =
    getRecordValue<unknown>(rawRule, "affectedPieces") ??
  ...
  if (
    ...
    Array.isArray(rawAffectedPieces) &&
    rawAffectedPieces.length > 0
  ) {
  ...
  const conditions = sanitizeConditions(
    getRecordValue<unknown>(rawRule, "conditions"),
    issues,
  );
  let effects = sanitizeEffects(
    getRecordValue<unknown>(rawRule, "effects"),
    issues,
  );
  ...
  effects = effects.map((effect) => {
    ...
    const abilityParameters = effect.parameters as
    ...
    const originalParameters = (effect.parameters ?? {}) as Record<
      string,
      unknown
    >;
    ...
    if (metadata) {
      if (
        originalParameters.radius !== undefined &&
        updatedParameters.radius !== originalParameters.radius
      ) {
        issues.push(
          `Rayon invalide pour l'attaque spéciale ${metadata.label} normalisé.`,
        );
      }
      if (
        originalParameters.countdown !== undefined &&
        updatedParameters.countdown !== originalParameters.countdown
      ) {
        issues.push(
          `Compte à rebours invalide pour l'attaque spéciale ${metadata.label} ajusté.`,
        );
      }
      if (
        originalParameters.damage !== undefined &&
        updatedParameters.damage !== originalParameters.damage
      ) {
        issues.push(
          `Dégâts invalides pour l'attaque spéciale ${metadata.label} ajustés.`,
        );
      }
      if (
        originalParameters.trigger !== undefined &&
        updatedParameters.trigger !== originalParameters.trigger
      ) {
        issues.push(
          `Mode de déclenchement invalide pour l'attaque spéciale ${metadata.label} réinitialisé.`,
        );
      }
    }
  ...
  const rawIsActive =
    getRecordValue<unknown>(rawRule, "isActive") ??
    getRecordValue<unknown>(rawRule, "is_active");
  const isActive =
    typeof rawIsActive === "boolean" ? rawIsActive : Boolean(rawIsActive);
  ...
  const validationRules = (() => {
    const rawValidation =
      getRecordValue<unknown>(rawRule, "validationRules") ??
    ...
    const allowedWithSource = getRecordValue<unknown>(
      rawValidation,
      "allowedWith",
    );
    const conflictsWithSource = getRecordValue<unknown>(
      rawValidation,
      "conflictsWith",
    );
    const requiredState =
      getRecordValue<unknown>(rawValidation, "requiredState") ?? null;
    ...
    if (
      hasProvidedListValues(conflictsWithSource) &&
  ...
  type MetadataKey = keyof Pick<
    ChessRule,
    "assets" | "uiActions" | "parameters" | "state"
  >;
  ...
  if (isRecord(rawRule)) {
    const metadataSource = rawRule as Record<string, unknown>;
    const finalRuleWithMetadata = finalRule as ChessRule & {
      __originalRuleJson?: unknown;
    };
    const metadataTarget = finalRuleWithMetadata as Partial<
      Record<MetadataKey, unknown>
    >;

    metadataKeys.forEach((key) => {
      const value = metadataSource[key];
      if (value !== undefined) {
        metadataTarget[key] = value;
      }
    });

    if ("__originalRuleJson" in metadataSource) {
      finalRuleWithMetadata.__originalRuleJson =
        metadataSource.__originalRuleJson;
    }
  }

  if (isRecord(rule)) {
    const rawId = getRecordValue<unknown>(
      rule as Record<string, unknown>,
      "id",
    );
    ...
    const rawUserId =
      getRecordValue<unknown>(rule as Record<string, unknown>, "userId") ??
    ...
    const rawCreatedAt =
      getRecordValue<unknown>(rule as Record<string, unknown>, "createdAt") ??
    ...
    const rawUpdatedAt =
      getRecordValue<unknown>(rule as Record<string, unknown>, "updatedAt") ??
