...
// UI complète : assistant + saisie + config locale + rendu résultat.
...
// Rappels de structure pour guider le modèle (strict-JSON)
const SYSTEM_PROMPT =
...
type HeuristicsPayload = {
  // Ce bloc peut rester minimal pour la v1 client-side ; si tu veux,
  // tu peux y injecter les sorties du pipeline heuristique côté front plus tard.
  instruction: string;
};
...
const ConfigBlock: React.FC<{
  ...
  >
    <h3 style={{ marginTop: 0 }}>Configuration du service local</h3>
...
const VariantsIA: React.FC = () => {
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Ici tu peux déclencher une validation locale, puis sauvegarder en base si OK.
      // ex: await saveRuleToSupabase(res.rule)
    } catch (e) {
    ...
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "24px 16px" }}>
      ...
      <AssistantBubble>
        ...
        <div style={{ opacity: 0.9 }}>
          Décrivez votre idée de règle d’échecs. Je poserai des questions
          complémentaires avant de proposer la règle finale.
        </div>
      </AssistantBubble>
