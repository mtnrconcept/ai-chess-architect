...
import { applyMoveToGameState, canPlayFromState } from "@/lib/gameMoveState";
import {
  GameState,
  Position,
  ChessPiece,
...
const Play = () => {
  ...
  }, [gameState.currentPlayer, onTurnStart]);
  ...
  useEffect(() => {
    ...
    if (last.promotion) {
      onPromote(movedPieceId, "pawn", last.promotion);
    }
  }, [
    boardAdapter,
  ...
  const generateRule = useCallback(
    async (payload: {
      ...
      if (
        ...
        payload.prompt.trim().length > 6_000
      ) {
        safeToast({
          title: "Requête incomplète",
          description: "La règle doit contenir entre 8 et 6 000 caractères.",
          variant: "destructive",
        });
        return;
      }
      ...
      navigate("/generator", {
        state: { ruleIdeaDraft: payload.prompt.trim() },
      });
    },
    [navigate, safeToast],
  );
  ...
  const handleSquareClick = useCallback(
    (position: Position) => {
      ...
      setGameState((prev) => {
        if (!canPlayFromState(prev)) return prev;
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-6">
        ...
        <section className="grid grid-cols-1 items-start gap-4 lg:grid-cols-3">
          ...
          <div className="rounded-lg border border-white/10 p-4">
            ...
            >
              <label htmlFor="play-rule-prompt" className="sr-only">
                Décrivez la règle à concevoir
              </label>
              <Input
                id="play-rule-prompt"
                name="prompt"
                ...
                maxLength={6_000}
                required
                placeholder="Décrivez votre règle..."
              />
              <Button type="submit">
