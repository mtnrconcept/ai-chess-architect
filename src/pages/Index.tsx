      ...
      <div className="relative z-10 flex min-h-screen flex-col">
        <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-16 px-6 py-12">
          <header className="flex flex-col items-center gap-6 text-center">
            <div className="space-y-3">
              <span className="text-xs font-semibold uppercase tracking-[0.35em] text-cyan-200/80">Voltus-Chess · v9 Final</span>
              <h1 className="text-5xl font-bold leading-tight sm:text-6xl md:text-7xl">
                ...
                <br />
                <span className="text-white/80">des variantes assistées par IA</span>
              </h1>
              <p className="mx-auto max-w-2xl text-base text-cyan-100/70 sm:text-lg">
                Composez vos règles, rejoignez la communauté et lancez instantanément des parties ultra rapides dans un hub futuriste.
              </p>
            </div>
            ...
            {navItems.map(item => {
              ...
              return (
                <Link key={item.label} to={to} className="group relative">
                  <div className={`absolute inset-0 rounded-2xl opacity-0 transition-opacity duration-200 group-hover:opacity-100 bg-gradient-to-r ${item.accent}`} />
                  ...
                  <div className={`pointer-events-none absolute inset-x-[18%] bottom-0 h-[3px] rounded-full bg-gradient-to-r ${item.accent}`} />
                </Link>
              );
            })}
          </section>
          ...
          <section className="grid gap-10 lg:grid-cols-[3fr_2fr]">
            <div className="relative overflow-hidden rounded-3xl border border-cyan-500/20 bg-black/50 p-8 shadow-[0_0_55px_rgba(34,211,238,0.25)] backdrop-blur-xl">
              ...
              <div className="flex flex-col gap-6">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <span className="text-xs font-semibold uppercase tracking-[0.35em] text-cyan-200/80">Variantes en tendance</span>
                    <h2 className="mt-2 text-3xl font-bold text-white">Explorer les mondes parallèles</h2>
                  </div>
                  <Link to="/analysis">
                    <Button variant="ghost" className="rounded-full border border-cyan-400/30 bg-cyan-500/10 text-cyan-100 transition-all hover:border-cyan-300 hover:bg-cyan-500/20">
                      Voir l'analyse
                    </Button>
                  </Link>
                </div>
                  ...
                  {trendingVariants.map(variant => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <h3 className="mt-4 text-xl font-semibold">{variant.title}</h3>
                        <p className="mt-2 text-sm text-cyan-100/70">{variant.description}</p>
            ...
            <div className="flex flex-col gap-6 rounded-3xl border border-amber-400/20 bg-gradient-to-b from-black/60 via-black/40 to-black/60 p-8 shadow-[0_0_45px_rgba(251,191,36,0.2)] backdrop-blur-xl">
              <div className="flex flex-col gap-2 text-left">
                <span className="text-xs font-semibold uppercase tracking-[0.35em] text-amber-200/70">Partie instantanée</span>
                <h2 className="text-3xl font-bold text-white">Choisissez votre copilote</h2>
                <p className="text-sm text-amber-100/70">
                  Passez de l'entraînement assisté par l'IA aux défis contre des adversaires humains.
                </p>
              </div>
              <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/40 p-3 shadow-inner shadow-amber-400/10">
                ...
                >
                  <span className="text-xs uppercase tracking-[0.3em]">Joueur</span>
              ...
              <div className="flex items-center justify-between text-xs text-amber-100/70">
                <span>Mode sélectionné : {mode === "ia" ? "Entraînement IA" : "Défi joueur"}</span>
        ...
        <footer className="border-t border-white/10 bg-black/30 py-6">
          <div className="mx-auto flex w-full max-w-6xl flex-col items-center gap-2 px-6 text-center text-xs text-white/60 sm:flex-row sm:justify-between">
            <p>© {new Date().getFullYear()} Voltus-Chess. Propulsé par Lovable Cloud & Lovable AI.</p>
