...
- **Projet Supabase explicitement confirmé** – les scripts d'écriture n'ont aucune cible implicite et exigent
  `SUPABASE_PROJECT_ID`, la même valeur dans `SUPABASE_PROJECT_REF_CONFIRMATION`, puis une URL correspondant exactement à cette cible.
  Le frontend contient uniquement des cibles et clés publiques de secours, verrouillées sur les identités Vercel/Git exactes de
  staging et production ; toute variable `VITE_SUPABASE_*` explicite les remplace et toute configuration partielle échoue fermée.
...
Définis `SUPABASE_DB_URL` avec une URL Postgres TLS du projet visé, sans jamais la committer. Les variables de confirmation ci-dessus sont obligatoires.
...
La commande encapsule une version épinglée de `supabase db push` et refuse toute cible qui ne correspond pas à la triple confirmation.

L'alias historique suivant utilise exactement le même chemin sécurisé :

```bash
pnpm run db:migrate
```

Il ne maintient aucun registre parallèle de migrations : l'historique officiel Supabase reste l'unique source de vérité.
...
`pnpm run build` est volontairement pur et ne déclenche aucune écriture distante. Pour appeler explicitement un webhook Lovable, exécute `pnpm deploy:lovable` avec l'une des variables suivantes :

- `LOVABLE_DEPLOY_HOOK`
- `LOVABLE_DEPLOY_URL`
- `LOVABLE_DEPLOY_ENDPOINT`

Le script exige HTTPS hors localhost, masque chemin et paramètres dans les logs et ne journalise jamais le corps de réponse. Des options supplémentaires sont disponibles :
...
En l'absence de hook, la commande explicite se termine sans écriture distante.
