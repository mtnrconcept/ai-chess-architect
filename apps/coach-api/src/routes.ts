...
export function createCoachRouter(options: CoachApiOptions = {}) {
  ...
  routes.push(
    createRoute("POST", "/games/ingest", async (ctx) => {
  ...
  routes.push(
    createRoute("POST", "/analyses/:gameId/queue", async (ctx) => {
      const ownerId = getOwnerFromRequest(ctx.request);
  ...
  routes.push(
    createRoute("GET", "/analyses/:gameId/status", async (ctx) => {
      const ownerId = getOwnerFromRequest(ctx.request);
      const status = await service.getAnalysisStatus(
        ctx.params.gameId,
        ownerId,
      );
  ...
  routes.push(
    createRoute("GET", "/analyses/:gameId/report", async (ctx) => {
      const ownerId = getOwnerFromRequest(ctx.request);
  ...
  routes.push(
    createRoute("POST", "/analyses/:gameId/rebuild", async (ctx) => {
  ...
  routes.push(
    createRoute("GET", "/moves/:gameId", async (ctx) => {
      const ownerId = getOwnerFromRequest(ctx.request);
...
class InMemoryCoachService implements CoachService {
  private readonly store = new Map<
    string,
    { ownerId: string; moves: MoveRecord[]; analysis?: AnalysisResult }
  >();
